...
% h1 = [kernel((ks+1)/2:end) zeros(1,M-ks) kernel(1:(ks+1)/2-1)];
h2 = [kernel((ks+1)/2:end) zeros(1,N-(ks+1)/2)];
% h2 = [kernel((ks+1)/2:end) zeros(1,N-ks) kernel(1:(ks+1)/2-1)];
B1 = toeplitz(h1,h1); 
B2 = toeplitz(h2,h2);
% D1 = kron(eye(M/dsRatio),[zeros((dsRatio-1)/2,1);1;zeros((dsRatio-1)/2,1)]);
D1 = kron(eye(M/dsRatio),[1;zeros(dsRatio-1,1)]);
% D2 = kron(eye(N/dsRatio),[zeros((dsRatio-1)/2,1);1;zeros((dsRatio-1)/2,1)]);
D2 = kron(eye(N/dsRatio),[1;zeros(dsRatio-1,1)]);
B = KernelToMatrix(kernel,M,N);
G = sparse(kron(B2*D2,B1*D1));
% B_LRSR = sparse(kron(B2,B1));
S = sparse(kron(D2,D1));
end
