...
if( isfield( opt , 'AStep'        ) ) ; AStep        = opt.AStep        ; else AStep        = 100                                                   ; end ;
if( isfield( opt , 'SStep'        ) ) ; SStep        = opt.SStep        ; else SStep        = 100                                                   ; end ;
...
for itera = 1:maxIteraNum
    ...
    for i = 1:SStep
        ...
        S    = max( S  .*S_d ./S_n  , eps ) ;
        multiObjCurr = 0.5 * sum(sum( (Y_M-FA*S).^2 )) ;
        if( (multiObjCurr-multiObjPrev)/multiObjCurr < convTh_multi )
            break ;
        end
    end
    SG = S * G ;
    for i = 1:AStep
        ...
        A    = max( A .*A_d ./A_n  , eps ) ;
        hyperObjCurr = 0.5 * sum(sum( (Y_H-A*SG).^2 )) ;
        if( (hyperObjCurr-hyperObjPrev)/hyperObjCurr < convTh_hyper )
            break ;
        end
    end
    obj_prev = obj_curr ;
